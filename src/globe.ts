...
let earth: any;
let arcGroup: any;
let planeMesh: any = null;
let tooltip: HTMLDivElement;
const pointer = new THREE.Vector2();
const raycaster = new THREE.Raycaster();

let altitudeRange: [number, number] = [0, Infinity];
let pointSize = TUBE_RADIUS;
let flightInfos: Array<{ icao24: string; callsign: string; lon: number; lat: number; alt: number }> = [];
const prevPositions: Map<string, { lon: number; lat: number; alt: number }> = new Map();
...
export function setAltitudeFilter(min: number, max: number): void {
  altitudeRange[0] = min;
  altitudeRange[1] = max;
}

export function updateFlights(flights: any[]): void {
  arcGroup.clear();
  if (planeMesh) {
  ...
  const validFlights: Array<{ icao24: string; callsign: string; lon: number; lat: number; alt: number }> = [];
  ...
  for (const info of validFlights) {
    ...
    const mat = new THREE.MeshBasicMaterial({ color: colorScale(info.alt) as any });
